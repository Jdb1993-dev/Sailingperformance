...
- **GPS** (telefoon): actuele snelheid (SOG) en koers (COG). Eén gedeelde GPS-verbinding voor de hele app (niet per scherm); bij terugkomst uit de achtergrond wordt meteen een snelle (mogelijk iets verouderde) positie opgehaald en de verbinding herstart, i.p.v. te wachten op een verse hoge-nauwkeurigheid fix. Bij een geweigerde locatietoestemming volgt een pop-up met (Android/iOS-specifieke) instructies + een "Probeer opnieuw"-knop.
  - *Sneller een positie*: de laatst bekende positie wordt bewaard (localStorage) en direct getoond bij het openen, zodat koers/afstand naar je boei meteen zichtbaar zijn terwijl de echte fix binnenkomt; de eerste GPS-aanvraag pakt bovendien meteen een reeds door het OS gecachte positie (`maximumAge: Infinity`). Snelheid/koers worden pas getoond zodra er een echt (continu) spoor is — een oude/gecachte positie levert dus nooit een verzonnen snelheid op (delta-afleiding wordt overgeslagen bij een tijdsprong > 10s).
...
- **Boei (Course to Steer / DTW / VMG / ETA)**: zolang er geen boei gekozen is, tonen de CTS- en DTW-tegels "tik om boei te kiezen" (tik erop om een boei te zoeken uit `waypoints.gpx`). Zodra er één gekozen is, verschijnt die als kleine chip rechtsboven ("Navigeer naar: <boei> ⋮", tik om te wijzigen) en tonen de tegels: **Course to Steer** (kompaskoers naar de boei), **DTW** (afstand in nm, rechte lijn), **VMG** naar de boei (`SOG × cos(peiling − COG)`; positief = inlopen, negatief = eraf) met een klein trendteken (▲/▼/● t.o.v. ~10s geleden), en **ETA** (verwachte aankomsttijd bij de boei = afstand / VMG bij je huidige closing-speed; "--" als je niet inloopt).

Scherm-indeling (eerste scherm, uniforme tegels): rij 1 Target (Polar) | Actual speed · rij 2 Performance | VMG · rij 3 Course to Steer | Koers (COG) · rij 4 DTW | ETA · rij 5 TWA | TWS | True wind (drie naast elkaar).
...
- **Timer**: tik op de klok om een starttijd in te stellen — óf als vast tijdstip (uu:mm:ss, vandaag), óf als **countdown in minuten vanaf nu** (bv. "5" voor een startklok over 5 minuten). Beide zetten hetzelfde onderliggende doeltijdstip; een countdown van 5 minuten om 10:00 wordt dus gewoon opgeslagen als "10:05:00". Groot: minuten:seconden tot die tijd. Rechtsboven klein: de ingestelde tijd zelf. Wordt onthouden voor de rest van de dag (localStorage), maar niet meegenomen naar de volgende dag.
- **Startlijn**: "Pin punt A" en "Pin punt B" leggen de twee uiteinden van de startlijn vast op je huidige GPS-positie (bv. bij de pin-boei en het startschip langsvaren). Of tik op **"Kies op kaart..."** om A en B aan te wijzen op een satellietkaart (Leaflet + Esri World Imagery, gratis en zonder API-key — dus geen Google Maps-account nodig) — handig als je de lijn al vooraf kent en de boeien op de foto kan herkennen. Beide manieren zijn te combineren, en pinnen zijn achteraf te verslepen op de kaart om ze bij te stellen. Zodra beide gezet zijn:
...
Tegel-provider: Esri's gratis `World_Imagery` (satelliet, geen API-key). Eerder gebruikten we OSM's eigen `tile.openstreetmap.org` en daarna CARTO's straatkaart-tegels; OSM's eigen tegel-servers zijn volgens hun policy niet bedoeld voor gebruik in apps en bleken bij minstens één gebruiker geblokkeerd (terwijl een losse tegel-URL wel gewoon laadde in de browser) — vandaar de stap naar een provider die embedding wél toestaat.
