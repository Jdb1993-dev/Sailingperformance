...
- **Course to steer / Distance to waypoint / VMG**: zolang er geen boei gekozen is, tonen de CTS- en DTW-tegels "tik om boei te kiezen" (tik erop om een boei te zoeken uit `waypoints.gpx`). Zodra er één gekozen is, verschijnt die als kleine chip rechtsboven (met ⋮ om te wijzigen) en tonen de tegels de kompaskoers en afstand (nm) in rechte lijn naar die boei, plus de **VMG naar de boei** (snelheid richting de boei = `SOG × cos(peiling − COG)`; positief = je loopt in, negatief = je loopt eraf) met daarboven een klein teken of die VMG t.o.v. ~10s geleden beter (▲), slechter (▼) of gelijk (●) is.

Scherm-indeling (eerste scherm, van boven naar beneden): Performance % en Snelheid (GPS) groot naast elkaar; Koers (COG) als breed blok eronder; Target (polar) en VMG naar boei naast elkaar; Course to steer en Distance to WP naast elkaar; TWA, Wind (TWS) en Windrichting als drie blokken naast elkaar.
